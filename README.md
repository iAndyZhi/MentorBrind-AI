...
## 近期更新：群聊记录接入（2026-06-29）

知识库现已支持放在 Google Drive `chat history` 文件夹中的 TXT 群聊导出。引入群聊的目的，是补充整理笔记未覆盖的临场解释、案例和讨论，而不是让大量聊天片段取代经过整理的课程笔记。

本次更新采用“**整理笔记为主，Brind 群聊为辅**”的原则：

- 只把 Brind（张成熙）的发言作为导师知识；其他成员的发言仅提供少量提问语境。
- 自动忽略图片、视频、表情、过短寒暄、单独回应和转贴引用。
- 检索时默认给整理笔记保留 20 个候选席位，群聊保留 10 个；存在相关笔记时，最终最多采用 3 个群聊来源。
- 整理笔记和群聊出现冲突时，以整理笔记为准，并把群聊视为补充材料。
- 支持常见代称的双向理解，例如 `200斤` 与 `习近平`、`CCP/CPC/中共` 与 `中国共产党`。

这项更新会扩大可回答范围，但群聊本身仍可能存在口语省略和上下文缺失。重要判断请结合 Source，并回到原始笔记核对。聊天文件新增或替换后，需要点击侧栏 **Refresh**，等待知识索引重新变为 `Cached`。
